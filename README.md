...
Uses the pre-built image `ghcr.io/thefrizzy1/mcp-manager:latest` (published from
GitHub Actions). Update later with the same `pull && up -d`. To build locally
...
The dashboard's **Agents** panel runs a headless **Claude Code** agent that can operate
Plutus's own ~193 tools (e.g. *"find stuck \*arr queues and restart unhealthy
containers"*) with a live console and cost tracking — and lets you **schedule** agent
tasks or individual tool calls on a cron. Log Claude Code in once
