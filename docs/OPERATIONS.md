...
# edit docker-compose.yml: set any NAS volume path/UUID for your host
```

**Pull the pre-built image (no local build):**

```bash
docker compose pull && docker compose up -d
```

The compose file references `ghcr.io/thefrizzy1/mcp-manager:latest`, published by
the GitHub Actions workflow (`.github/workflows/docker-publish.yml`) on every push
to `main` and every `vX.Y.Z` tag. To update later: `docker compose pull && docker
compose up -d`.
...
**Agent runner:** to use the headless agent/scheduler, log Claude Code in once —
`docker exec -it plutus-mcp claude` (or set `ANTHROPIC_API_KEY`). See
[AGENTS.md](AGENTS.md).
